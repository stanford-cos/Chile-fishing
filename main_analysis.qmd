...
easter_vessels_map <- easter_data |> 
...
mapshot(easter_map, 
        file = file.path(out_dir, paste0("easter_isl_fish_effort_", timeframe, ".png")))
```
...
test <- ggplot(df,aes(x=date,
                      y=0,
                      color = best_vessel_class)) +
  labs(col="Vessel Class") +
  scale_color_manual(values=gear_color, drop = FALSE) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y", limits = c(ymd(as.character(year(min(df$date))), truncated = 2L), NA)) +
  theme_classic() +
