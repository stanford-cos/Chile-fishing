...
::: callout-note
Code runs but results not as expected (don't match map). Working to resolve. Summary currently not available
:::

```{r vessels_mpa}
# convert dataframe variables into sf coords
vessel_coords <- st_as_sf(easter_data, coords = c("lat","lon")) 
...
#mpa_vessels <- easter_data[unlist(lapply(ptsInt, length)) > 0, ]
...
# interactive html map
mpa_map <- leaflet(easter_vessels) |> 
  addProviderTiles(providers$Esri.WorldPhysical) |> 
    setView(lng = -104.485692, 
            lat = -26.547169, 
            #     setView(lng = -104.485692, 
            # lat = -26.547169, 
            zoom = 7) |> 
